                    ...
                    std::vector<std::string> additional_infos)
{
  ...
  // cout<<"end col: "<< GetColumnNumber(src_mgr_, end_loc)<<endl;
...
void MutantDatabase::ExportAllEntries()
{
  ...
  // WHEN LINE LIMIT IS GIVEN BY THE USER
  if (num_mutant_limit_on_line_ != -1) {
    for (auto line_map_iter: mutant_entry_table_) {
      ...
      for (auto column_map_iter: line_map_iter.second) {
        for (auto mutantname_map_iter: column_map_iter.second) {
          for (auto entry: mutantname_map_iter.second) {
            ...
            // only push back mutantname when the line_mutant_count is beloew num_mutant_limit_on_pos_
            if (line_mutant_count[mutantname_map_iter.first] < num_mutant_limit_on_pos_) {
              line_mutant_count[mutantname_map_iter.first] += 1;
              line_entries.push_back(make_pair(mutantname_map_iter.first, entry));
            }
          }
        }
      }

      // WHEN NUMBER OF MUTANTS ON A LINE IS LESS THAN OR EQUAL TO THE GIVEN LIMIT
      if (line_entries.size() <= num_mutant_limit_on_line_) {
        for (auto entry: line_entries) {
          WriteEntryToDatabaseFile(entry.first, entry.second);
          WriteEntryToMutantFile(entry.second);
          IncrementNextMutantfileId();
        ...
        for (auto idx: rand_entries) {
          WriteEntryToDatabaseFile(line_entries[idx].first, line_entries[idx].second);
          WriteEntryToMutantFile(line_entries[idx].second);
          IncrementNextMutantfileId();

          mutant_count[line_entries[idx].first] += 1;
        }
      }
    }
  } else {
    for (auto line_map_iter: mutant_entry_table_) {
      ...
      for (auto column_map_iter: line_map_iter.second) {
        for (auto mutantname_map_iter: column_map_iter.second)
        {
          ...
          if (mutantname_map_iter.second.size() <= num_mutant_limit_on_pos_)
          {
            for (auto entry: mutantname_map_iter.second)
            {
              WriteEntryToDatabaseFile(mutantname_map_iter.first, entry);
          ...
          else
          {
            ...
            set<int> random_nums;
            GenerateRandomNumbers(random_nums, num_mutant_limit_on_pos_, 
