...
bool InformationVisitor::VisitCXXRecordDecl(clang::CXXRecordDecl *d) {
  bool is_class = d->isClass();
  if (is_class) {
    std::set<clang::CXXMethodDecl*> methods;
    cla_to_methods_map_[d] = methods;
  }
  return true;
}
bool InformationVisitor::VisitCXXMethodDecl(clang::CXXMethodDecl* md) {
    // Check if the method is an overloaded operator
    if (md->isOverloadedOperator()) {
        // Get the class that declares this method
        clang::CXXRecordDecl* record = md->getParent();

        // Add the method to the set of methods for the class
        cla_to_methods_map_[record].insert(md);
    }
...
bool InformationVisitor::VisitFunctionDecl(FunctionDecl *fd)
{
      ...
      !LocationIsInRange(start_loc, *currently_parsed_function_range_))
  {
    ...
    if(fd->getTemplatedKind() == FunctionDecl::TK_FunctionTemplate){
      FunctionTemplateDecl* tmplDecl = fd->getDescribedFunctionTemplate();
      TemplateParameterList* paramList = tmplDecl->getTemplateParameters();
      for (auto param : *paramList) {
        if (auto typeParam = dyn_cast<TemplateTypeParmDecl>(param)) {
          if(typeParam->wasDeclaredWithTypename()){
            cout<< "found a template type: " << typeParam->getNameAsString() <<endl;
            for (auto it = fd->param_begin(); it != fd->param_end(); ++it)
            {
              // func_param_list_.back().push_back(*it);
              QualType paramType = (*it)->getType();
              if(paramType.getAsString() == typeParam->getNameAsString()){
                cout << "param that has this type: " << (*it)->getNameAsString() << endl;
              }
            }
          }
        }
...
bool InformationVisitor::VisitCallExpr(clang::CallExpr* ce) 
{
  SourceLocation start_loc = ce->getBeginLoc();
  SourceLocation end_loc = ce->getEndLoc();

  if (!ValidateSourceRange(start_loc, end_loc))
    return true;
  // Iterate over the arguments
  // cout<<"function: "<< ConvertToString(ce, comp_inst_->getLangOpts())<<endl;
  ...
  if (fd) {
    ...
    for (auto it = fd->param_begin(); it != fd->param_end(); it++) {
      ...
      // Expr *arg1 = (*it)->IgnoreImpCasts();
      // cout<<"###############"<<endl;
      // cout<<ConvertToString((*it), comp_inst_->getLangOpts())<<endl;
      const auto* argType = (*it)->getType().getTypePtr();

      // Check if the type is an l-value reference type
      if (argType->isLValueReferenceType()) {
        // cout <<index_arg<< "Argument is an l-value\n";
        if(fd->getNameAsString().find(">>") != string::npos){
          index_arg++;
          // cout<<"found"<<endl;
        }
        if (ce->getNumArgs() > index_arg - 1)
          ...
          continue;
        // arg->dump();
        // cout<<"expr: "<< ConvertToString(arg, comp_inst_->getLangOpts())<<endl;
        l_value_arg_list_.push_back(arg);        
      } else {
        // cout << "Argument is not an l-value\n";
      }
      index_arg++;
      // cout<<"###############"<<endl;
    }
...
SymbolTable* InformationVisitor::getSymbolTable()
{
  return new SymbolTable(
      ...
      &func_local_list_,&l_value_arg_list_, &case_value_list_, &line_to_vars_map_, &line_to_consts_map_, &cla_to_methods_map_,
