...
static llvm::cl::list<string> OptionD(
    "d", llvm::cl::desc("Specify mutant operator name to disable (not-use)"),
    llvm::cl::value_desc("[mutantname]"),
    llvm::cl::cat(MusicOptions));

static llvm::cl::opt<int> OptionLL(
    "ll", llvm::cl::desc("Specify maximum number of mutants generated per line"),
    llvm::cl::value_desc("maxnum"),
    llvm::cl::init(-1), llvm::cl::cat(MusicOptions));
...
void ParseOptionLL()
{
  ...
  if (OptionLL != -1)
  {
    ...
    {
      cout << "Invalid input for -ll option, must be an positive integer smaller than 4294967296\n";
      cout << "Usage: -ll <max>\n";
      exit(1);
    }
  }
...
void ParseOptionD()
{
  ...
  for (auto e: OptionD)
  {
    cout << "disabling " << e << endl;
    vector<string> mutant_operator;
    SplitStringIntoVector(e, mutant_operator, string(","));

    // Capitalize mutant operator name.
    for (int i=0; i<mutant_operator.size(); i++)
    {
      for (int j = 0; j < mutant_operator[i].length() ; ++j)
      {
        if (mutant_operator[i][j] >= 'a' && 
            mutant_operator[i][j] <= 'z')
          mutant_operator[i][j] -= 32;
      }
    }

    for (auto it: mutant_operator)
    {
      // Remove expr mutant operator from list if found.
      for (auto it2 = g_expr_mutant_operator_list.begin(); 
           it2 != g_expr_mutant_operator_list.end(); )
      {
        // make new string variable
        // and assign it2 string that is all capitalized
        string temp = (*it2)->getName();
        ...
        {
          it2 = g_expr_mutant_operator_list.erase(it2);
        }
        else
          ++it2;
      }

      // Remove stmt mutant operator from list if found.
      for (auto it2 = g_stmt_mutant_operator_list.begin(); 
           it2 != g_stmt_mutant_operator_list.end(); )
      {
        // make new string variable
        // and assign it2 string that is all capitalized
        string temp = (*it2)->getName();
        ...
        if (temp.compare(it) == 0)
        {
          it2 = g_stmt_mutant_operator_list.erase(it2);
        }
        else
          ++it2;
      }
