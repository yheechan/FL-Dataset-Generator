...
def start_process(subject_name):
    ...
    extract_sbfl_features(configs, subject_working_dir, distribution_machineCore2bugsList)
...
def extract_sbfl_features(configs, subject_working_dir, distribution_machineCore2bugsList):
    global bin_dir
    ...
    machine_bin_dir = base_dir + 'bin/05-2_extract_sbfl_features/'

    mbfl_extraction_cmd_dir = bin_dir / '05-2_extract_sbfl_features'
    assert mbfl_extraction_cmd_dir.exists(), f"Test mutants directory {mbfl_extraction_cmd_dir} does not exist"

    bash_file = open('02-1_extract_sbfl_features_on_distributed_machines.sh', 'w')
    ...
    cmd = ['chmod', '+x', '02-1_extract_sbfl_features_on_distributed_machines.sh']
    res = sp.call(cmd)

    # time.sleep(1)

    # cmd = ['./02-1_extract_sbfl_features_on_distributed_machines.sh']
