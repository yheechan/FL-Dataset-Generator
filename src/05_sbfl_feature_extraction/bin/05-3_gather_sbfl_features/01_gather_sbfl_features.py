...
gather_sbfl_features_dir = script_path.parent
bin_dir = gather_sbfl_features_dir.parent
...
def start_process(subject_name):
    ...
    gather_dir = initialize_sbfl_dir(configs, subject_working_dir)
...
def initialize_sbfl_dir(configs, subject_working_dir):
    gather_dir = subject_working_dir / 'sbfl_features'
...
def retrieve_buggy_versions_distributed_machines(configs, subject_working_dir, gather_dir, machine_cores_list):
    ...
    workers = f"{home_directory}{subject_name}-sbfl_feature_extraction/{subject_name}-working_directory/workers_extracting_sbfl_features/"

    bash_file = open('01-1_retrieve_sbfl_features.sh', 'w')
    ...
    cmd = ['chmod', '+x', '01-1_retrieve_sbfl_features.sh']
    res = sp.call(cmd)

    # time.sleep(1)

    # cmd = ['./01-1_retrieve_sbfl_features.sh']
