...
def initialize_working_directory(configs, subject_name):
    global prepare_prerequisites_dir, user_configs_dir, subjects_dir, external_tools_dir

    # Create working directory
    working_dir = prepare_prerequisites_dir / f'{subject_name}-working_directory'
    ...
    clean_build_file = new_configure_dir / clean_script
    ...
    # copy extractor executable to external_tools directory
    extractor_exec = external_tools_dir / 'extractor/extractor'
    assert extractor_exec.exists(), 'Extractor executable does not exist'

    cmd = ['cp', extractor_exec, ext_dir]
    res = sp.call(cmd)
    if res != 0:
        raise Exception('Failed to copy extractor executable to working directory')
    ...
    # cmd = ['cp', '-r', buggy_versions_dir, working_dir / 'buggy_versions_set']
    # res = sp.call(cmd)
    # if res != 0:
    #     raise Exception('Failed to copy buggy versions to working directory')
    
    # number_of_buggy_versions = len(list(buggy_versions_dir.iterdir()))
    # print(f'Number of buggy versions: {number_of_buggy_versions}')
