...
gather_mbfl_features_dir = script_path.parent
bin_dir = gather_mbfl_features_dir.parent
...
def start_process(subject_name):
    ...
    gather_dir = initialize_mbfl_dir(configs, subject_working_dir)
...
def initialize_mbfl_dir(configs, subject_working_dir):
    gather_dir = subject_working_dir / 'mbfl_features'
...
def retrieve_buggy_versions_distributed_machines(configs, subject_working_dir, gather_dir, machine_cores_list):
    ...
    workers = f"{home_directory}{subject_name}-mbfl_feature_extraction/{subject_name}-working_directory/workers_extracting_mbfl_features/"

    bash_file = open('01-1_retrieve_mbfl_features.sh', 'w')
    ...
    cmd = ['chmod', '+x', '01-1_retrieve_mbfl_features.sh']
    res = sp.call(cmd)

    # time.sleep(1)

    # cmd = ['./01-1_retrieve_mbfl_features.sh']
