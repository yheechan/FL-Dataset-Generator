...
gather_buggy_versinos_dir = script_path.parent
bin_dir = gather_buggy_versinos_dir.parent
...
def start_process(subject_name):
    ...
    buggy_versions_dir = initialize_buggy_versions_directory(configs, subject_working_dir)
    ...
    retrieve_buggy_versions(configs, subject_working_dir, buggy_versions_dir, machine_cores_list)

def initialize_buggy_versions_directory(configs, subject_working_dir):
    buggy_versions_dir = subject_working_dir / 'usable_buggy_versions'
    buggy_versions_dir.mkdir(exist_ok=True)
    return buggy_versions_dir

def retrieve_buggy_versions(configs, subject_working_dir, buggy_versions_dir, machine_cores_list):
    global use_distributed_machines

    if configs[use_distributed_machines] == True:
        retrieve_buggy_versions_distributed_machines(configs, subject_working_dir, buggy_versions_dir, machine_cores_list)
    else:
        retrieve_buggy_versions_single_machine(configs, subject_working_dir, buggy_versions_dir, machine_cores_list)

def retrieve_buggy_versions_distributed_machines(configs, subject_working_dir, buggy_versions_dir, machine_cores_list):
    home_directory = configs['home_directory']
    subject_name = configs['subject_name']
    workers = f"{home_directory}{subject_name}-select_usable_buggy_versions/{subject_name}-working_directory/workers_selecting_buggy_versions/"

    bash_file = open('01-1_retrieve_usable_buggy_versions.sh', 'w')
    ...
    for machine_core in machine_cores_list:
        ...
        machine_core_dir = f"{workers}{machine_id}/{core_id}/usable_buggy_versions/*"

        cmd = 'scp -r {}:{} {} & \n'.format(machine_id, machine_core_dir, buggy_versions_dir)
    ...
    cmd = ['chmod', '+x', '01-1_retrieve_usable_buggy_versions.sh']
    res = sp.call(cmd)

    # time.sleep(1)

    # cmd = ['./01-1_retrieve_usable_buggy_versions.sh']
    # print("Retrieving buggy mutants from workers...")
    # res = sp.call(cmd)


def retrieve_buggy_versions_single_machine(configs, subject_working_dir, buggy_mutant_dir, machine_cores_list):
    workers_dir = subject_working_dir / 'workers_selecting_buggy_versions'

    for machine_core in machine_cores_list:
        machine_id = machine_core.split(':')[0]
        core_id = machine_core.split(':')[1]
        machine_core_bug_dir = workers_dir.__str__() + f"/{machine_id}/{core_id}/usable_buggy_versions/*"
